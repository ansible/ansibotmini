...
* closed issues are not processed, the only exception being to lock closed issues/PRs after 14 days
* adds [Labels](#labels)
* supports [Commands](#commands)
* automatically identifies components (files in `ansible/ansible`) that relate to an issue/PR
* a listing of all issues/PRs per a file of the `ansible/ansible` repository is periodically generated and can be accessed at https://ansibotmini.core.ansible.com
...
* auto-cancels CI runs on Azure Pipelines for pull requests that are not based on a branch from a fork and such pull requests are auto-closed
...
| `bot_closed`             | Issues and pull requests | Indicates an issue/PR has been closed by the bot, see `needs_info`, `needs_template` and `waiting_on_contributor` [Labels](#labels) and `!needs_collection_redirect` [Command](#commands).                                                                                                                                                                                 |
...
| `needs_template`         | Issues                   | This issue has an incomplete description. Please open a new issue and fill in the appropriate template.                                                                                                                                                                                                                                                  |
...
| `pending_ci`             | Pull requests            | The CI is stuck and needs to be re-run.                                                                                                                                                                                                                                                                                                                  |
...
| `stale_pr`               | Pull requests            | This PR has not been pushed to for more than one year.                                                                                                                                                                                                                                                                                                   |
| `stale_review`           | Pull requests            | Updates were made after the last review.                                                                                                                                                                                                                                                                                                                 |
...
| `@ansibot !needs_collection_redirect` | Issues                   | Anyone    | Use this command if bot made a mistake in deciding an issue was for a file in a collection. Ansible Core team member will need to re-open the issue. Contact a Core team member to review the issue on https://forum.ansible.com.                                                   |
...
Issues and pull requests data are cached in `~/.ansibotmini_cache.pickle`.
